...
const Home = () => {
    const { data: blogs, isPending, error } = useFetch('http://localhost:8000/blogs') // use useFetch file in this component and send the url as a parameter
    ...
    return (
        <div className="home">
            {error && <div>{error}</div>}
            {isPending && <div>Loading...</div>} {/* <-- show this div only when isPending state is true */}
